...
async function main() {
  ...
  console.log("========================================");
  console.log("06_deploy_validation.js");
  ...
  const CityValidation = await ethers.getContractFactory("CityValidation");
  const cityValidation = await CityValidation.deploy(
    deployer.address,
    cityConfigAddress,
    cityRegistryAddress,
    { nonce: nextNonce++ }
  );
  await cityValidation.waitForDeployment();

  const cityValidationAddress = await cityValidation.getAddress();
  console.log("CityValidation deployed:", cityValidationAddress);
  ...
  const merged = {
    ...
    cityValidation: cityValidationAddress,
  ...
  console.log("========================================");
  console.log("CityValidation fertig.");
  console.log("JSON aktualisiert:", coreFile);
  console.log("========================================");
}

main().catch((error) => {
  console.error("Fehler in 06_deploy_validation.js");
  console.error(error);
  process.exitCode = 1;
});