...
async function main() {
  ...
  console.log("========================================");
  console.log("04_deploy_status.js");
  ...
  const CityStatus = await ethers.getContractFactory("CityStatus");
  const cityStatus = await CityStatus.deploy(
    deployer.address,
    cityConfigAddress,
    cityRegistryAddress,
    { nonce: nextNonce++ }
  );
  await cityStatus.waitForDeployment();

  const cityStatusAddress = await cityStatus.getAddress();
  console.log("CityStatus deployed:", cityStatusAddress);
  ...
  console.log("========================================");
  console.log("CityStatus fertig.");
  console.log("JSON aktualisiert:", coreFile);
  console.log("========================================");
}

main().catch((error) => {
  console.error("Fehler in 04_deploy_status.js");
  console.error(error);
  process.exitCode = 1;
});